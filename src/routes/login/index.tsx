...
function LoginPage() {
  ...
  const handleSocialError = (err: any, providerName: string) => {
    ...
    if (err.code === "auth/unauthorized-domain") {
      ...
      setError(
        `${providerName} login is not enabled in your Firebase project. Please enable it in the Firebase Console.`,
      );
      ...
      setError(
        `${providerName} sign-in failed. Please try again or use Demo Mode.`,
      );
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        setError(
          "Invalid credentials. Use Demo Mode to explore the platform without an account.",
        );
      } else if (err.code === "auth/too-many-requests") {
        setError(
          "Too many failed attempts. Try again later or continue with Demo Mode.",
        );
      } else {
        setError(
          "Authentication failed. Use Demo Mode if the service is restricted in your environment.",
        );
      }
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-navy-900 flex flex-col justify-center items-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md animate-slide-up relative z-10">
        ...
        <div className="glass p-8 rounded-3xl border border-white/10 shadow-2xl relative">
          {error && (
            <div
              className={`mb-6 p-4 rounded-xl border text-sm ${isDomainError ? "bg-amber-500/10 border-amber-500/20 text-amber-400" : "bg-red-500/10 border-red-500/20 text-red-400"}`}
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center ml-1">
                ...
                <button
                  type="button"
                  className="text-[10px] text-accent-indigo font-bold hover:text-accent-mint transition-colors tracking-widest uppercase"
                >
                  Forgot?
                </button>
              </div>
