...
const SignupPage: React.FC = () => {
  ...
  const handleDemoMode = () => {
    const userData = {
      ...
      name: formData.firstName ? `${formData.firstName} ${formData.lastName}` : "Guest Pilot",
      plan: "pro",
      isDemo: true,
  ...
  const handleSocialError = (err: any, providerName: string) => {
    ...
    if (err.code === 'auth/unauthorized-domain') {
      ...
      setError(`${providerName} signup is not enabled in your Firebase project. Please enable it in the Firebase Console.`);
    } else if (err.code === 'auth/popup-blocked') {
      setError("Pop-up was blocked. Please enable pop-ups for this domain to sign up.");
    } else {
      setError(`${providerName} signup failed. Please try another method or use Demo Mode.`);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message.includes("email-already-in-use") || err.code === 'auth/email-already-in-use') {
        ...
        setError("Account creation failed. Use Demo Mode to explore the platform features.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-navy-900 flex flex-col justify-center items-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md animate-slide-up relative z-10">
        <div className="flex flex-col items-center mb-8 text-center">
          ...
          <p className="text-gray-400">Start building smarter proposals today</p>
        </div>

        <div className="glass p-8 rounded-3xl border border-white/10 shadow-2xl">
          {error && (
            <div className={`mb-6 p-4 rounded-xl border text-sm flex flex-col gap-3 ${emailInUse ? 'bg-accent-indigo/10 border-accent-indigo/20 text-accent-indigo' : isDomainError ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
